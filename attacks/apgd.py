...
class APGDAttack():
    def __init__(self, model, n_iter=100, norm='Linf', n_restarts=1, eps=None,
                 seed=0, loss='ce', eot_iter=1, rho=.75, n_class=10, verbose=False,
                 device='cuda'):
        self.model = model
        self.n_iter = n_iter
        self.eps = eps
    ...
    def attack_single_run(self, x_in, y_in, criterion=None):
        x = x_in.clone() if len(x_in.shape) == 4 else x_in.clone().unsqueeze(0)
        y = y_in.clone() if len(y_in.shape) == 1 else y_in.clone().unsqueeze(0)

        self.n_iter_2, self.n_iter_min, self.size_decr = max(int(0.22 * self.n_iter), 1), max(int(0.06 * self.n_iter), 1), max(
        ...
        elif self.loss == 'searched' and criterion is not None:
            criterion_indiv = criterion
        else:
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                if self.loss == 'searched':
                    loss_indiv = criterion_indiv(logits, F.one_hot(y, num_classes=self.n_class))
                    loss_indiv = loss_indiv.sum(dim=1)
                else:
        ...
        step_size = self.eps * torch.ones([x.shape[0], 1, 1, 1]).to(self.device).detach() * torch.Tensor([2.0]).to(self.device).detach().reshape(
            [1, 1, 1, 1])
        ...
        for i in range(self.n_iter):
            ...
            with torch.no_grad():
                ...
                if self.norm == 'Linf':
                    ...
                    x_adv_1 = torch.clamp(x + (x_adv_1 - x) / (((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt() + 1e-12) * torch.min(
                        self.eps * torch.ones(x.shape).to(self.device).detach(), ((x_adv_1 - x) ** 2).sum(dim=(1, 2, 3), keepdim=True).sqrt()), 0.0,
                                          1.0)
            ...
            with torch.no_grad():
                ...
                if counter3 == k:
                    fl_oscillation = self.check_oscillation(loss_steps.detach().cpu().numpy(), i, k, loss_best.detach().cpu().numpy(),
                                                            k3=self.thr_decr)
    ...
    def perturb(self, x_in, y_in, best_loss=False, cheap=True, criterion=None):
        ...
        if not best_loss:
            ...
            if not cheap:
                ...
                for counter in range(self.n_restarts):
                    ...
                    if ind_to_fool.numel() != 0:
                        ...
                        best_curr, acc_curr, loss_curr, adv_curr = self.attack_single_run(x_to_fool, y_to_fool, criterion)
            ...
            for counter in range(self.n_restarts):
                best_curr, _, loss_curr, _ = self.attack_single_run(x, y, criterion)
